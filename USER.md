# USER.md - About Your Human

_Learn about the person you're helping. Update this as you go._

- **Name:** Dylan Grabowski
- **What to call them:** Dylan
- **Date of Birth:** _July 11, 1986_
- **Timezone:** America/Denver (MDT)
- **Location:** Colorado

## Context

### Work & Projects
- Is a writer and editor for www.NeoNewsToday.com
- Is the host of www.SmartEconomyPodcast.com
- Conducts interviews and livestreams at https://www.youtube.com/@grabowskidylan
- Can be found on X/Twitter at https://x.com/GrabowskiDylan
- Runs a daily crypto livestream and needs a 9am MDT briefing to prepare
- Building a corpus of podcast interview transcripts and written articles in Obsidian
- Obsidian vault located at ~/.openclaw-vault/
- Daily crypto briefing appended to ~/`.openclaw-vault/Streams/Daily Digital Download.md`

## Career Knowledge Files
- `memory/EXPERTISE-MAP` — content domains, opinions, skills, notable guests, target roles
- `memory/CAREER-ACTIVITY` — panels, speaking, advocacy, conferences, booths, grants

### Job Search
- Actively looking for new opportunities
- Target areas: Ecosystem Growth Lead / Manager, Developer Relations (DevRel) Manager, Partner Success / Ecosystem Partnerships, Head of Content / Editorial Director, Crypto Media Editor / Senior Writer, Podcast Producer / Host (in-house), Community Manager / Director, DAO Operations Lead, Grants Program Manager, Policy Analyst / Government Relations (Web3), Crypto Advocacy Coordinator, Public Affairs Manager, Events & Conferences Lead, Brand Ambassador / Ecosystem Representative, Head of BD & Events, Govtech / Smart City Blockchain Consulting, Infrastructure Tokenization Advisor, Digital Identity Policy roles 
- Help needed with: finding listings, tailoring applications, drafting materials
- Where to look:
	- Job boards: Crypto Jobs List, Web3.career, Greenhouse (for protocol companies), LinkedIn filtered to Web3/crypto
	- Networks: ETH Denver, Consensus, your existing Neo/COZ/Stand With Crypto contacts
	- Direct targets: Ethereum Foundation, Polygon, Chainlink, Alchemy, Coinbase, Uniswap Foundation, Hedera, or any L1 blockchain that needs ecosystem/content/policy people

### Communication Style
- Direct and efficient — no filler
- Prefers structured, sourced information
- Not intimidated by technical detail

---

The more you know, the better you can help. But remember — you're learning about a person, not building a dossier. Respect the difference.
